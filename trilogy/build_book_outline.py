#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""通用四级目录渲染器（书→章→节→目）。用法：python3 build_book_outline.py <outline.json>
读 _atlas/<outline.json>（book/en/subtitle/axiom/arc_label/arc/model/standard/stats/chapters）
→ 墨绿鎏金 HTML + 同源 MD。判断/渲染分离·任意书共用。资产页另有 build_zichan_outline.py（保持不动）。"""
import json, html, pathlib, sys
HERE = pathlib.Path(__file__).parent
name = sys.argv[1] if len(sys.argv) > 1 else "chuangye-outline.json"
D = json.load(open(HERE / "_atlas" / name, encoding="utf-8"))
...
def ch_block(c):
  ...
  <summary><span class="seal">{cn(c["seq"])}</span>
    <span class="ch-main"><span class="ch-n">{e(c.get("title") or ("第"+cn(c["seq"])+"章"))} · {e(c["name"])}</span>
      <span class="ch-layer">{e(c["layer"])}</span><span class="ch-q">{e(c["q"])}</span></span>
...
S = D["stats"]; fab = st.get("fabricated", [])
fabline = f'<span class="ok-dot">臆造 {len(fab)}</span>' if not fab else f'<span class="no-dot">疑似臆造 {len(fab)}</span>'
en = D.get("en", "YUANLI · 4-LEVEL OUTLINE")
arc_label = D.get("arc_label", "通关链")

HTML = f'''<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1.0"/>
<title>《{e(D["book"])}》· 四级目录 · 金标准</title>
<style>
...
.arc{{margin:16px auto 0;font-family:var(--sans);font-size:12px;color:var(--muted);max-width:780px;line-height:2}}
...
.std-h .no-dot{{font-family:var(--sans);font-size:11px;color:var(--cinnabar-soft);border:1px solid var(--cinnabar);border-radius:10px;padding:1px 8px}}
...
.note{{max-width:790px;margin:22px auto 0;font-family:var(--sans);font-size:12px;color:var(--muted);border:1px dashed var(--line);border-radius:10px;padding:13px 18px;line-height:1.8}}
.note b{{color:var(--gold-light)}}
@media(max-width:680px){{.checks{{grid-template-columns:1fr}}.ch-c{{display:none}}}}
</style></head><body>
<header>
  <div class="kick">{e(en)}</div>
  <h1 class="foil">《{e(D["book"])}》· 四级目录</h1>
  <div class="sub-t">{e(D.get("subtitle",""))} · 书 → 章 → 节 → 目 · 金标准</div>
  <div class="ax">公理 <b>{e(D.get("axiom",""))}</b>{(" · 模型＝" + e(D["model"])) if D.get("model") else ""}</div>
  <div class="arc">{e(arc_label)}：{arc}<br/>{e(D.get("arc",""))}</div>
  <div class="kpis"><span class="kpi"><b>{S["chapters"]}</b> {e(D.get("ch_unit","章"))}</span><span class="kpi"><b>{S["sections"]}</b> 节</span><span class="kpi"><b>{S["subs"]}</b> 目</span><span class="kpi">五标准 <b>{"全过" if st.get("verdict")=="pass" else st.get("verdict","")}</b></span></div>
</header>
<div class="wrap">
  <div class="std">
    <div class="std-h">五标准校验 <span class="vd">{e(st.get("verdict","")).upper()}</span> {fabline}</div>
    <div class="checks">{checks}</div>
    <div class="weak"><b>最弱节（诚实标注）</b> {e(st.get("weakest",""))}</div>
  </div>
  {chapters}
  <div class="note"><b>{e(D.get("note","这是逐节写正文的写作单元真源。"))}</b></div>
  <footer>
    <div>《{e(D["book"])}》四级目录 · 判断/渲染分离（{e(name)} + build_book_outline.py）· 墨绿鎏金</div>
    <div style="margin-top:8px"><a href="../index.html">← 返回三部曲门户</a> · <a href="原力三部曲-提纲架构.html">提纲架构</a> · <a href="原力资产-四级目录.html">原力资产四级目录</a></div>
  </footer>
</div></body></html>'''

slug = D["book"]
(HERE / f"{slug}-四级目录.html").write_text(HTML, encoding="utf-8")
print(f"wrote {slug}-四级目录.html | {S['chapters']} 章 / {S['sections']} 节 / {S['subs']} 目 | verdict={st.get('verdict')}")

# 同源 MD
md = [f"# 《{D['book']}》· 四级目录（书 → 章 → 节 → 目）· 金标准\n",
      f"> {D.get('subtitle','')} · 公理 **{D.get('axiom','')}**" + (f" · 模型＝{D['model']}" if D.get('model') else "") + "  ",
      f"> {arc_label}：{' → '.join(c['name'] for c in D['chapters'])}。{D.get('arc','')}  ",
      f"> 规模：**{S['chapters']} 章 / {S['sections']} 节 / {S['subs']} 目**；五标准**{'全过' if st.get('verdict')=='pass' else st.get('verdict')}**（critic verdict=`{st.get('verdict')}`，疑似臆造 {len(fab)}）。",
      "\n> **五标准** —— " + " · ".join(f"{k} {'✓' if ok else '✕'}" for k, ok, _ in st["checks"]),
      f"\n> **最弱节**：{st.get('weakest','')}\n", "---\n"]
for c in D["chapters"]:
    md.append(f"## {c.get('title') or ('第'+cn(c['seq'])+'章')} · {c['name']}　〔{c['layer']}〕")
    ...
    for i, sec in enumerate(c["sections"], 1):
        ...
        md.append("")
    md.append("")
md.append("---\n*逐节写正文的写作单元真源。每个「目」≈ 一个可直接落笔的写作单元。*")
(HERE / f"{slug}-四级目录.md").write_text("\n".join(md), encoding="utf-8")
print(f"wrote {slug}-四级目录.md")
